...
- 第一跳允许 query key, 因为微信内链接无法携带 header。
- key 校验通过后创建短期 grant。
- 重定向到微信 OAuth `snsapi_base`。

`GET /wx/oauth/callback?code=...&state=...`
...
- `/wx/bind?key=...` 允许 query key。
- 成功后立即换成 HMAC state。
- state 包含 nonce、purpose、expires_at。
- auth_grant 只能使用一次。
