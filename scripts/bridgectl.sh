...
ENV_FILE="${ENV_FILE:-${BRIDGE_CONFIG_ENV_FILE:-/etc/sage-wiki-bridge.env}}"
if [ -f "$ENV_FILE" ]; then
  # shellcheck disable=SC1090
  . "$ENV_FILE"
fi

BRIDGE_BIN_PATH="${BRIDGE_BIN_PATH:-${BIN_PATH:-./target/release/sage-wiki-bridge}}"
...
BRIDGE_APP_VERSION="${BRIDGE_APP_VERSION:-$( "$BRIDGE_BIN_PATH" --version 2>/dev/null || printf '%s' unknown )}"
