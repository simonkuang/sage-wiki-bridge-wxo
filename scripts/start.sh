...
BRIDGE_WECHAT_OAUTH_AUTHORIZE_BASE="${BRIDGE_WECHAT_OAUTH_AUTHORIZE_BASE:-https://open.weixin.qq.com/connect/oauth2/authorize}"
BRIDGE_WECHAT_TOKEN_REFRESH_SKEW_SECONDS="${BRIDGE_WECHAT_TOKEN_REFRESH_SKEW_SECONDS:-300}"
BRIDGE_WHITELIST_JOIN_REDIRECT_URL="${BRIDGE_WHITELIST_JOIN_REDIRECT_URL:-}"
...
set -- "$@" \
  ...
  --wechat-oauth-authorize-base "$BRIDGE_WECHAT_OAUTH_AUTHORIZE_BASE" \
  --wechat-token-refresh-skew-seconds "$BRIDGE_WECHAT_TOKEN_REFRESH_SKEW_SECONDS" \
  --admin-base-path "$BRIDGE_ADMIN_BASE_PATH" \
  --whitelist-join-redirect-url "$BRIDGE_WHITELIST_JOIN_REDIRECT_URL" \
